...
export const formatDateTimeToISO = (dateTimeString) => {
  // change date format from dd/mm/yyyy to yyyy-mm-ddThh:MM:ss to put in new Date()
...
export const getLoggedInUser = () => {
  ...
  const user = api.UserAPI.get_by_id(loggedInUserId);
  return user;
};

export const validateLogin = (username, password) => {
  const users = api.UserAPI.filter({
    username: username,
    password: password,
  });
  if (users.length) {
